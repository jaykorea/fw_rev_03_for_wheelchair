...
if (!input_scan.ranges.empty()) {
    ...
    float deg_65 = floor(res_per_deg * (float)65.0);

    for (unsigned int i = int(floor(las_mid_ran - deg_65)); i < int(floor(las_mid_ran+deg_65)); i++) { // left view of lidar frame <---|(0 degree)
