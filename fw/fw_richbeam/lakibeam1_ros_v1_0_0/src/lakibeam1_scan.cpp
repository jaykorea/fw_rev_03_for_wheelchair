...
int main(int argc, char **argv)
{
    ...
    nh.getParam("print_debug", print_debug_);
    ...
    ROS_INFO("print_debug:%s", print_debug_);
	...
	while (ros::ok())
	{
		...
		if(scan_vec_ready == 1)
		{
			...
			if(print_debug_) ROS_INFO("New topic %s published, total data points: %d", output_topic.c_str(), num_readings);
			scan_vec.clear();
			scan_vec_ready = 0;
		}
	}
